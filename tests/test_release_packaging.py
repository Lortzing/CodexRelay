...
def test_package_release_creates_windows_zip(tmp_path: Path) -> None:
    ...
    archive = output / "codex-relay-1.2.3-windows-x86_64.zip"
    assert archive.is_file()
    with zipfile.ZipFile(archive) as handle:
        names = set(handle.namelist())
    root = "codex-relay-1.2.3-windows-x86_64"
    assert f"{root}/cxr.exe" in names
...
def test_package_release_creates_unix_tarball(tmp_path: Path) -> None:
    ...
    archive = output / "codex-relay-1.2.3-macos-arm64.tar.gz"
    assert archive.is_file()
    with tarfile.open(archive, "r:gz") as handle:
        members = {member.name: member for member in handle.getmembers()}
    root = "codex-relay-1.2.3-macos-arm64"
    assert f"{root}/cxr" in members
    assert members[f"{root}/cxr"].mode & 0o111
    assert f"{root}/INSTALL.txt" in members
    assert f"{root}/README.md" in members
