...
未配置 Authenticode 证书时，文件名会带 `-unsigned`，Windows SmartScreen 可能显示未知发布者警告。
...
未配置 Apple Developer ID 与公证凭据时，DMG 文件名会带 `-unsigned`，Gatekeeper 可能阻止直接打开。
...
签名需要你自己的 Windows 代码签名证书和 Apple Developer ID 私钥，无法由仓库代为生成。Workflow 已接入自动签名与 macOS 公证；配置方法见 [签名与公证说明](docs/SIGNING.md)。
